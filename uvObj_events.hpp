...
namespace uvObj {
    ...
    template <typename self_t, typename uvobj_t>
    struct events_t : events_std_t< self_t > {
        typedef events_obj_t< self_t, uvobj_t > ex;
    };

    struct BlackHoleSink {
        typedef events_std_t< BlackHoleSink > evt;

        void on_connect(int status) {}
        void on_shutdown(int status) {}
        void on_udp_send(int status) {}
        void on_write(int status) {}
        void on_work() {}
        void on_after_work() {}
        void on_getaddrinfo(int status, struct addrinfo* res) {}
